<script lang="ts">
	...
	const { form, errors, message, enhance, delayed, options, constraints } =
		superForm(data.form, {
			resetForm: true,
...
<section
	...
	<form class="p-4 mt-10 flex px-4" method="POST" id="form" use:enhance>
		<div class="flex flex-col space-y-6">
				...
				<div class="flex flex-col lg:flex-row">
					<div class="">
						...
						<div class="flex flex-col md:flex-row mb-6 mx-3 gap-6">
							<div class="w-full mb-6 md:w-1/2 md:mb-0 mt-5">
								<label class="block mb-2 text-xs" for="email"
									>Email {#if $errors.email}
										<span class="text-xs italic text-red-500"
											>{$errors.email}</span>
									{/if}</label>
								<input
							...
							<div class="w-full md:w-1/2 mt-5">
								<label class="block mb-2 text-xs" for="telephone"
									>Telefon {#if $errors.telephone}
										<span class="text-xs italic text-red-500"
											>{$errors.telephone}</span>
									{/if}</label>
								<input
									class="block w-full px-4 py-3"
									id="telephone"
					...
					<div>
						<div class="w-full px-3">
							<label class="block mb-2 text-xs" for="text"
								>Zpráva{#if $errors.text}
									<span class="text-xs italic text-red-500"
										>{$errors.text}</span>
								{/if}</label>
							<textarea
			...
			<span class="flex justify-end mr-3 text-sm flex-wrap">
				Odesláním souhlasíte <span class="whitespace-nowrap"
					>&nbsp;se&nbsp;</span>
				<a href="/gdpr" class="underline whitespace-nowrap"
